...
export const CartItems = () => {
  const {all_product, getTotalCartAmount, selectedSizes, cartItems, removeFromCart, cartCount} = useContext(ShopContext);
                ...
                return (
                  <div key={item.id}>
                    <div className='w-[260px] lg:w-auto grid lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr_1fr_1fr] items-center gap-[75px] text-gray-700'>
                      ...
                      <p>{item.name}</p>
                      <p>${item.new_price}</p>
                      <p>{selectedSizes[item.id] ? selectedSizes[item.id].toUpperCase() : "Not Selected"}</p>
